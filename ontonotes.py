    ...
    def _conll_rows_to_sentence(self, conll_rows: List[str]) -> OntonotesSentence:
        ...
        for index, row in enumerate(conll_rows):
            ...
            if pos_tag != "XX" and word != "XX":
                ...
                if pos_tag == ")":
                    pos_tag = "-RRB-"
                (left_brackets, right_hand_side) = ('_', '_')
            ...
            self._process_coref_span_annotations_for_word(
                conll_components[10], index, clusters, coref_stacks
            )

            sentence.append(word)
            pos_tags.append(pos_tag)
            parse_pieces.append(parse_piece)
            predicate_lemmas.append(lemmatised_word if lemmatised_word != "_" else None)
            predicate_framenet_ids.append(framenet_id if framenet_id != "_" else None)
            word_senses.append(float(word_sense) if word_sense != "_" else None)
            speakers.append(speaker if speaker != "_" else None)
